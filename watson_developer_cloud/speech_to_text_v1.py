...
"""
### Service Overview  The IBM Speech to Text service provides a Representational State
Transfer (REST) Application Programming Interface (API) that enables you to add IBM's
speech transcription capabilities to your applications. The service also supports an
asynchronous HTTP interface for transcribing audio via non-blocking calls. And it supports
a customization interface that lets you expand the vocabulary of a base model with
domain-specific terminology or adapt a base model for the acoustic characteristics of your
audio.   The service transcribes speech from various languages and audio formats to text
with low latency. The service supports transcription of the following languages: Brazilian
...
broadband and narrowband.  ### API Overview The Speech to Text service provides the
following endpoints: * `/v1/models` returns information about the language models that are
available for speech recognition. * `/v1/recognize` (sessionless) includes a single method
that provides a simple means of transcribing audio without the overhead of establishing
and maintaining a session, but it lacks some of the capabilities available with sessions.
* `/v1/sessions` provides a collection of methods that provide a mechanism for a client to
maintain a long, multi-turn exchange, or session, with the service or to establish
multiple parallel conversations with a particular instance of the service. *
...
terminology. * `/v1/customizations/{customization_id}/corpora` provides methods for
managing the corpora associated with a custom language model. You add corpora to extract
...
custom language model. * `/v1/acoustic_customizations` provides methods for creating and
managing custom acoustic models. The interface lets you adapt a base model for the audio
characteristics of your environment and speakers. *
...
can add, list, and delete audio resources from a custom acoustic model.    **Note about
the Try It Out feature:** The `Try it out!` button lets you experiment with the methods of
the API by making actual cURL calls to the service. The feature is **not** supported for
use with the session-based `POST /v1/sessions/{session_id}/recognize` and sessionless
`POST /v1/recognize` methods. For examples of calls to these methods, see the [Speech to
Text API reference](http://www.ibm.com/watson/developercloud/speech-to-text/api/v1/).  ###
API Usage The following information provides details about using the service to transcribe
audio: * **HTTP REST interfaces:** You can use methods of the session-based, sessionless,
or asynchronous HTTP interfaces to pass audio data to the service. All interfaces let you
...
you control the transcription primarily via JSON metadata sent as form data. * **WebSocket
interface:** The service also offers a WebSocket interface as an alternative to its HTTP
interfaces. The WebSocket interface supports efficient implementation, lower latency, and
higher throughput. The interface establishes a persistent connection with the service,
eliminating the need for session-based calls from the HTTP interface. See [The WebSocket
...
**Audio transmission:** You can pass the audio to be transcribed as a one-shot delivery or
in streaming mode. With one-shot delivery, you pass all of the audio data to the service
at one time. With streaming mode, you send audio data to the service in chunks over a
persistent connection. To use streaming, you must pass the `Transfer-Encoding` request
header with a value of `chunked`. Both forms of data transmission impose a limit of 100 MB
of total data for transcription. See [Audio
...
Customization API Usage   The following information pertains to methods of the
customization interface: * Language model customization and acoustic model customization
are available only for a limited set of languages. They are generally available for
production use for some languages but are beta offerings for other languages. For a
complete list of supported languages and the status of their availability, see [Language
support for
...
class SpeechToTextV1(WatsonService):
    ...
    def recognize_sessionless(self, model=None, customization_id=None, acoustic_customization_id=None, customization_weight=None, version=None, audio=None, content_type=None, inactivity_timeout=None, keywords=None, keywords_threshold=None, max_alternatives=None, word_alternatives_threshold=None, word_confidence=None, timestamps=None, profanity_filter=None, smart_formatting=None, speaker_labels=None):
        ...
        `version`, `inactivity_timeout`, `keywords`, `keywords_threshold`,
        `max_alternatives`, `word_alternatives_threshold`, `word_confidence`,
        `timestamps`, `profanity_filter`, `smart_formatting`, and `speaker_labels`
        ###Multipart requests   For multipart requests, you specify a few parameters of
        the request as request headers and query parameters, but you specify most
        parameters as multipart form data in the form of JSON metadata, in which only
        `part_content_type` is required. You then specify the audio files for the request
        as subsequent parts of the form data. Use this approach with browsers that do not
        support JavaScript or when the parameters of the request are greater than the 8 KB
        limit imposed by most HTTP servers and proxies. Use the following parameters: *
        **Required:** `Content-Type`, `metadata`, and `upload` * **Optional:**
        `Transfer-Encoding`, `model`, `customization_id`, `acoustic_customization_id`,
        `customization_weight`, and `version`   An example of the multipart metadata for a
        pair of FLAC files follows. This first part of the request is sent as JSON; the
        remaining parts are the audio files for the request.
        `metadata=\"{\\\"part_content_type\\\":\\\"audio/flac\\\",\\\"data_parts_count\\\":2,\\\"inactivity_timeout\\\"=-1}\"`
          **Note about the Try It Out feature:** The `Try it out!` button is **not**
        supported for use with the the `POST /v1/recognize` method. For examples of calls
        to the method, see the [Speech to Text API
    ...
    def check_jobs(self):
        ...
        status is `completed` or not one of the latest 100 outstanding jobs, use the `GET
        /v1/recognitions/{id}` method. A job and its results remain available until you
        delete them with the `DELETE /v1/recognitions/{id}` method or until the job's time
    ...
    def create_language_model(self, name, base_model_name, dialect=None, description=None):
        """
    ...
    def reset_language_model(self, customization_id):
        ...
        if customization_id is None:
            raise ValueError('customization_id must be provided')
        url='/v1/customizations/{0}/reset'.format(*self._encode_path_vars(customization_id))
        self.request(method='POST',
            url=url,
            accept_json=True)
        return None
    ...
    def train_language_model(self, customization_id, word_type_to_add=None, customization_weight=None):
        """
        ...
        * One or more words that were added to the custom model have invalid sounds-like
        ...
        url='/v1/customizations/{0}/train'.format(*self._encode_path_vars(customization_id))
        self.request(method='POST',
            url=url,
            params=params,
            accept_json=True)
    ...
    def upgrade_language_model(self, customization_id):
        ...
        status of the upgrade by using the `GET /v1/customizations/{customization_id}`
        method to poll the model's status. Use a loop to check the status every 10
        seconds. While it is being upgraded, the custom model has the status `upgrading`.
        When the upgrade is complete, the model resumes the status that it had prior to
        upgrade. The service cannot accept subsequent requests for the model until the
        upgrade completes.   For more information, see [Upgrading custom
        ...
        if customization_id is None:
            raise ValueError('customization_id must be provided')
        url='/v1/customizations/{0}/upgrade_model'.format(*self._encode_path_vars(customization_id))
        self.request(method='POST',
            url=url,
            accept_json=True)
        return None
    ...
    def add_corpus(self, customization_id, corpus_name, corpus_file, allow_overwrite=None, corpus_file_content_type=None, corpus_filename=None):
        """
        ...
        url='/v1/customizations/{0}/corpora/{1}'.format(*self._encode_path_vars(customization_id, corpus_name))
        self.request(method='POST',
            url=url,
            params=params,
    ...
    def list_corpora(self, customization_id):
        ...
        if customization_id is None:
            raise ValueError('customization_id must be provided')
        url='/v1/customizations/{0}/corpora'.format(*self._encode_path_vars(customization_id))
        response = self.request(method='GET',
            url=url,
            accept_json=True)
        return response
    ...
    def add_word(self, customization_id, word_name, word=None, sounds_like=None, display_as=None):
        """
        ...
        * The `display_as` field provides a different way of spelling the word in a
        ...
        url='/v1/customizations/{0}/words/{1}'.format(*self._encode_path_vars(customization_id, word_name))
    ...
    def add_words(self, customization_id, words):
        ...
        * The `display_as` field provides a different way of spelling the word in a
        ...
        url='/v1/customizations/{0}/words'.format(*self._encode_path_vars(customization_id))
        self.request(method='POST',
            url=url,
            json=data,
            accept_json=True)
    ...
    def delete_acoustic_model(self, customization_id):
        ...
        if customization_id is None:
            raise ValueError('customization_id must be provided')
        url='/v1/acoustic_customizations/{0}'.format(*self._encode_path_vars(customization_id))
        self.request(method='DELETE',
            url=url,
            accept_json=True)
        return None
    ...
    def reset_acoustic_model(self, customization_id):
        ...
        if customization_id is None:
            raise ValueError('customization_id must be provided')
        url='/v1/acoustic_customizations/{0}/reset'.format(*self._encode_path_vars(customization_id))
        self.request(method='POST',
            url=url,
            accept_json=True)
        return None
    ...
    def train_acoustic_model(self, customization_id, custom_language_model_id=None):
        """
        ...
        training takes approximately two to four times the length of its audio data. The
        range of time depends on the model being trained and the nature of the audio, such
        as whether the audio is clean or noisy. The method returns an HTTP 200 response
        code to indicate that the training process has begun.   You can monitor the status
        of the training by using the `GET /v1/acoustic_customizations/{customization_id}`
        method to poll the model's status. Use a loop to check the status once a minute.
        The method returns an `Customization` object that includes `status` and `progress`
        fields. A status of `available` indicates that the custom model is trained and
        ready to use. The service cannot accept subsequent training requests, or requests
        to add new audio resources, until the existing request completes.   You can use
        the optional `custom_language_model_id` query parameter to specify the GUID of a
        separately created custom language model that is to be used during training.
        ...
        url='/v1/acoustic_customizations/{0}/train'.format(*self._encode_path_vars(customization_id))
        self.request(method='POST',
            url=url,
            params=params,
            accept_json=True)
    ...
    def upgrade_acoustic_model(self, customization_id, custom_language_model_id=None):
        """
        ...
        successfully. You can monitor the status of the upgrade by using the `GET
        /v1/acoustic_customizations/{customization_id}` method to poll the model's status.
        ...
        subsequent requests for the model until the upgrade completes.   If the custom
        acoustic model was trained with a separately created custom language model, you
        must use the `custom_language_model_id` query parameter to specify the GUID of
        that custom language model. The custom language model must be upgraded before the
        custom acoustic model can be upgraded. Omit the parameter if the custom acoustic
        model was not trained with a custom language model.   For more information, see
        [Upgrading custom
        models](https://console.bluemix.net/docs/services/speech-to-text/custom-upgrade.html).
        ...
        url='/v1/acoustic_customizations/{0}/upgrade_model'.format(*self._encode_path_vars(customization_id))
        self.request(method='POST',
            url=url,
            params=params,
            accept_json=True)
    ...
    def add_audio(self, customization_id, audio_name, audio_resource, content_type, contained_content_type=None, allow_overwrite=None):
        """
        ...
        **Note:** The sampling rate of an audio file must match the sampling rate of the
        ...
        data = audio_resource
        url='/v1/acoustic_customizations/{0}/audio/{1}'.format(*self._encode_path_vars(customization_id, audio_name))
        self.request(method='POST',
            url=url,
            headers=headers,
            params=params,
    ...
    def list_audio(self, customization_id):
        ...
        if customization_id is None:
            raise ValueError('customization_id must be provided')
        url='/v1/acoustic_customizations/{0}/audio'.format(*self._encode_path_vars(customization_id))
        response = self.request(method='GET',
            url=url,
            accept_json=True)
        return response
...
class AcousticModel(object):
    ...
    def __init__(self, customization_id, created=None, language=None, versions=None, owner=None, name=None, description=None, base_model_name=None, status=None, progress=None, warnings=None):
        """
    ...
    def _from_dict(cls, _dict):
        ...
        if 'customization_id' in _dict:
            ...
            raise ValueError('Required property \'customization_id\' not present in AcousticModel JSON')
        if 'created' in _dict:
            args['created'] = _dict['created']
        if 'language' in _dict:
            args['language'] = _dict['language']
        if 'versions' in _dict:
            args['versions'] = _dict['versions']
        if 'owner' in _dict:
    ...
    def _to_dict(self):
        ...
        _dict = {}
        if hasattr(self, 'customization_id') and self.customization_id is not None:
        ...
        if hasattr(self, 'base_model_name') and self.base_model_name is not None:
...
class AcousticModels(object):
    ...
    def _from_dict(cls, _dict):
        ...
        if 'customizations' in _dict:
            args['customizations'] = [AcousticModel._from_dict(x) for x in _dict['customizations']]
        else:
            raise ValueError('Required property \'customizations\' not present in AcousticModels JSON')
        return cls(**args)
    ...
    def _to_dict(self):
        ...
        if hasattr(self, 'customizations') and self.customizations is not None:
            _dict['customizations'] = [x._to_dict() for x in self.customizations]
        return _dict
...
class AudioDetails(object):
    ...
    def _from_dict(cls, _dict):
        ...
        if 'type' in _dict:
            ...
            raise ValueError('Required property \'type\' not present in AudioDetails JSON')
        if 'codec' in _dict:
...
class AudioListing(object):
    ...
    def __init__(self, duration=None, name=None, details=None, status=None, container=None, audio=None):
        """
    ...
    def _from_dict(cls, _dict):
        ...
        if 'audio' in _dict:
            args['audio'] = [AudioResource._from_dict(x) for x in _dict['audio']]
        return cls(**args)
...
class AudioResource(object):
    ...
    def _from_dict(cls, _dict):
        ...
        if 'duration' in _dict:
            ...
            raise ValueError('Required property \'duration\' not present in AudioResource JSON')
        if 'name' in _dict:
            args['name'] = _dict['name']
        else:
            raise ValueError('Required property \'name\' not present in AudioResource JSON')
        if 'details' in _dict:
            args['details'] = AudioDetails._from_dict(_dict['details'])
        else:
            raise ValueError('Required property \'details\' not present in AudioResource JSON')
        if 'status' in _dict:
            args['status'] = _dict['status']
        else:
            raise ValueError('Required property \'status\' not present in AudioResource JSON')
        return cls(**args)
...
class AudioResources(object):
    ...
    def _from_dict(cls, _dict):
        ...
        if 'total_minutes_of_audio' in _dict:
            ...
            raise ValueError('Required property \'total_minutes_of_audio\' not present in AudioResources JSON')
        if 'audio' in _dict:
            args['audio'] = [AudioResource._from_dict(x) for x in _dict['audio']]
        else:
            raise ValueError('Required property \'audio\' not present in AudioResources JSON')
        return cls(**args)
...
class Corpora(object):
    ...
    def _from_dict(cls, _dict):
        ...
        if 'corpora' in _dict:
            ...
            raise ValueError('Required property \'corpora\' not present in Corpora JSON')
        return cls(**args)
...
class Corpus(object):
    ...
    def __init__(self, name, total_words, out_of_vocabulary_words, status, error=None):
        """
    ...
    def _from_dict(cls, _dict):
        ...
        if 'name' in _dict:
            ...
            raise ValueError('Required property \'name\' not present in Corpus JSON')
        if 'total_words' in _dict:
            args['total_words'] = _dict['total_words']
        else:
            raise ValueError('Required property \'total_words\' not present in Corpus JSON')
        if 'out_of_vocabulary_words' in _dict:
            args['out_of_vocabulary_words'] = _dict['out_of_vocabulary_words']
        else:
            raise ValueError('Required property \'out_of_vocabulary_words\' not present in Corpus JSON')
        if 'status' in _dict:
            args['status'] = _dict['status']
        else:
            raise ValueError('Required property \'status\' not present in Corpus JSON')
        if 'error' in _dict:
...
class KeywordResult(object):
    ...
    def _from_dict(cls, _dict):
        ...
        if 'normalized_text' in _dict:
            ...
            raise ValueError('Required property \'normalized_text\' not present in KeywordResult JSON')
        if 'start_time' in _dict:
            args['start_time'] = _dict['start_time']
        else:
            raise ValueError('Required property \'start_time\' not present in KeywordResult JSON')
        if 'end_time' in _dict:
            args['end_time'] = _dict['end_time']
        else:
            raise ValueError('Required property \'end_time\' not present in KeywordResult JSON')
        if 'confidence' in _dict:
            args['confidence'] = _dict['confidence']
        else:
            raise ValueError('Required property \'confidence\' not present in KeywordResult JSON')
        return cls(**args)
...
class LanguageModel(object):
    ...
    def __init__(self, customization_id, created=None, language=None, dialect=None, versions=None, owner=None, name=None, description=None, base_model_name=None, status=None, progress=None, warnings=None):
        """
    ...
    def _from_dict(cls, _dict):
        ...
        if 'customization_id' in _dict:
            ...
            raise ValueError('Required property \'customization_id\' not present in LanguageModel JSON')
        if 'created' in _dict:
            args['created'] = _dict['created']
        if 'language' in _dict:
            args['language'] = _dict['language']
...
class LanguageModels(object):
    ...
    def _from_dict(cls, _dict):
        ...
        args = {}
        if 'customizations' in _dict:
            args['customizations'] = [LanguageModel._from_dict(x) for x in _dict['customizations']]
        else:
            raise ValueError('Required property \'customizations\' not present in LanguageModels JSON')
        return cls(**args)
...
class RecognitionJob(object):
    ...
    def __init__(self, id, status, created, updated=None, url=None, user_token=None, results=None, warnings=None):
        """
    ...
    def _from_dict(cls, _dict):
        ...
        if 'id' in _dict:
            ...
            raise ValueError('Required property \'id\' not present in RecognitionJob JSON')
        if 'status' in _dict:
            args['status'] = _dict['status']
        else:
            raise ValueError('Required property \'status\' not present in RecognitionJob JSON')
        if 'created' in _dict:
            args['created'] = _dict['created']
        else:
            raise ValueError('Required property \'created\' not present in RecognitionJob JSON')
        if 'updated' in _dict:
        ...
        if 'results' in _dict:
            args['results'] = [SpeechRecognitionResults._from_dict(x) for x in _dict['results']]
        if 'warnings' in _dict:
            args['warnings'] = _dict['warnings']
        return cls(**args)
...
class RecognitionJobs(object):
    ...
    def _from_dict(cls, _dict):
        ...
        if 'recognitions' in _dict:
            args['recognitions'] = [RecognitionJob._from_dict(x) for x in _dict['recognitions']]
        else:
...
class RegisterStatus(object):
    ...
    def _from_dict(cls, _dict):
        ...
        if 'status' in _dict:
            ...
            raise ValueError('Required property \'status\' not present in RegisterStatus JSON')
        if 'url' in _dict:
            args['url'] = _dict['url']
        else:
            raise ValueError('Required property \'url\' not present in RegisterStatus JSON')
        return cls(**args)
...
class SpeechModel(object):
    ...
    def __init__(self, name, language, rate, url, supported_features, description, sessions=None):
        """
    ...
    def _from_dict(cls, _dict):
        ...
        if 'name' in _dict:
            ...
            raise ValueError('Required property \'name\' not present in SpeechModel JSON')
        if 'language' in _dict:
            args['language'] = _dict['language']
        else:
            raise ValueError('Required property \'language\' not present in SpeechModel JSON')
        if 'rate' in _dict:
            args['rate'] = _dict['rate']
        else:
            raise ValueError('Required property \'rate\' not present in SpeechModel JSON')
        if 'url' in _dict:
            args['url'] = _dict['url']
        else:
            raise ValueError('Required property \'url\' not present in SpeechModel JSON')
        if 'supported_features' in _dict:
            args['supported_features'] = SupportedFeatures._from_dict(_dict['supported_features'])
        else:
            raise ValueError('Required property \'supported_features\' not present in SpeechModel JSON')
        if 'description' in _dict:
            args['description'] = _dict['description']
        else:
            raise ValueError('Required property \'description\' not present in SpeechModel JSON')
    ...
    def _to_dict(self):
        ...
        if hasattr(self, 'url') and self.url is not None:
            _dict['url'] = self.url
        if hasattr(self, 'supported_features') and self.supported_features is not None:
...
class SpeechModels(object):
    ...
    def _from_dict(cls, _dict):
        ...
        args = {}
        if 'models' in _dict:
            args['models'] = [SpeechModel._from_dict(x) for x in _dict['models']]
        else:
            raise ValueError('Required property \'models\' not present in SpeechModels JSON')
        return cls(**args)
...
class SpeechRecognitionAlternative(object):
    ...
    def __init__(self, transcript, confidence=None, timestamps=None, word_confidence=None):
        """
    ...
    def _from_dict(cls, _dict):
        ...
        if 'transcript' in _dict:
            ...
            raise ValueError('Required property \'transcript\' not present in SpeechRecognitionAlternative JSON')
        if 'confidence' in _dict:
            args['confidence'] = _dict['confidence']
    ...
    def _to_dict(self):
        ...
        if hasattr(self, 'word_confidence') and self.word_confidence is not None:
...
class SpeechRecognitionResult(object):
    ...
    :attr dict keywords_result: (optional) A dictionary (or associative array) whose keys are the strings specified for `keywords` if both that parameter and `keywords_threshold` are specified. A keyword for which no matches are found is omitted from the array. The array is omitted if no keywords are found.
    ...
    def __init__(self, final_results, alternatives, keywords_result=None, word_alternatives=None):
        """
        ...
        :param dict keywords_result: (optional) A dictionary (or associative array) whose keys are the strings specified for `keywords` if both that parameter and `keywords_threshold` are specified. A keyword for which no matches are found is omitted from the array. The array is omitted if no keywords are found.
    ...
    def _from_dict(cls, _dict):
        ...
        if 'final' in _dict:
            ...
            raise ValueError('Required property \'final\' not present in SpeechRecognitionResult JSON')
        if 'alternatives' in _dict:
            args['alternatives'] = [SpeechRecognitionAlternative._from_dict(x) for x in _dict['alternatives']]
        else:
            raise ValueError('Required property \'alternatives\' not present in SpeechRecognitionResult JSON')
        if 'keywords_result' in _dict:
            args['keywords_result'] = _dict['keywords_result']
        if 'word_alternatives' in _dict:
            args['word_alternatives'] = [WordAlternativeResults._from_dict(x) for x in _dict['word_alternatives']]
        return cls(**args)
    ...
    def _to_dict(self):
        ...
        if hasattr(self, 'keywords_result') and self.keywords_result is not None:
            _dict['keywords_result'] = self.keywords_result
        if hasattr(self, 'word_alternatives') and self.word_alternatives is not None:
            _dict['word_alternatives'] = [x._to_dict() for x in self.word_alternatives]
        return _dict
...
class SpeechRecognitionResults(object):
    ...
    def __init__(self, results=None, result_index=None, speaker_labels=None, warnings=None):
        """
    ...
    def _from_dict(cls, _dict):
        ...
        args = {}
        if 'results' in _dict:
            args['results'] = [SpeechRecognitionResult._from_dict(x) for x in _dict['results']]
        if 'result_index' in _dict:
            args['result_index'] = _dict['result_index']
        if 'speaker_labels' in _dict:
            args['speaker_labels'] = [SpeakerLabelsResult._from_dict(x) for x in _dict['speaker_labels']]
        if 'warnings' in _dict:
            args['warnings'] = _dict['warnings']
        return cls(**args)
...
class SupportedFeatures(object):
    ...
    def _from_dict(cls, _dict):
        ...
        if 'custom_language_model' in _dict:
            ...
            raise ValueError('Required property \'custom_language_model\' not present in SupportedFeatures JSON')
        if 'speaker_labels' in _dict:
            args['speaker_labels'] = _dict['speaker_labels']
        else:
            raise ValueError('Required property \'speaker_labels\' not present in SupportedFeatures JSON')
        return cls(**args)
...
class Word(object):
    ...
    def __init__(self, word, sounds_like, display_as, count, source, error=None):
        """
...
class WordAlternativeResult(object):
    ...
    def _from_dict(cls, _dict):
        ...
        if 'confidence' in _dict:
            ...
            raise ValueError('Required property \'confidence\' not present in WordAlternativeResult JSON')
        if 'word' in _dict:
            args['word'] = _dict['word']
        else:
            raise ValueError('Required property \'word\' not present in WordAlternativeResult JSON')
        return cls(**args)
...
class WordAlternativeResults(object):
    ...
    def _from_dict(cls, _dict):
        ...
        if 'start_time' in _dict:
            ...
            raise ValueError('Required property \'start_time\' not present in WordAlternativeResults JSON')
        if 'end_time' in _dict:
            args['end_time'] = _dict['end_time']
        else:
            raise ValueError('Required property \'end_time\' not present in WordAlternativeResults JSON')
        if 'alternatives' in _dict:
            args['alternatives'] = [WordAlternativeResult._from_dict(x) for x in _dict['alternatives']]
        else:
            raise ValueError('Required property \'alternatives\' not present in WordAlternativeResults JSON')
        return cls(**args)
...
class WordError(object):
    ...
    def _from_dict(cls, _dict):
        ...
        if 'element' in _dict:
            ...
            raise ValueError('Required property \'element\' not present in WordError JSON')
        return cls(**args)
...
class Words(object):
    ...
    def _from_dict(cls, _dict):
        ...
        if 'words' in _dict:
            ...
            raise ValueError('Required property \'words\' not present in Words JSON')
        return cls(**args)
